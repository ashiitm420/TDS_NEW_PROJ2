...
# The code below assumes a Unix-style /tmp directory. On Windows this usually
# does not exist, leading to FileNotFoundError when attempting to write files
# like /tmp/broken_down_tasks.txt. We proactively create it when running on
# Windows so existing path literals continue to work without a large refactor.
if os.name == "nt":
    try:
        os.makedirs("/tmp", exist_ok=True)
    except Exception as e:
        # As a fallback, if creating C:/tmp fails (rare), we fall back to the
        # system temp directory and set an environment variable so future
        # improvements could switch to using it. For now, the /tmp path is
        # created in almost all normal Windows environments (C:\tmp).
        import tempfile
        fallback_tmp = tempfile.mkdtemp(prefix="dataanalyst_")
        os.environ["FALLBACK_TMP_DIR"] = fallback_tmp
        print(f"⚠ Could not create /tmp, using fallback temp dir: {fallback_tmp}")
...
@app.options("/aianalyst/")
async def aianalyst_options():
...
# Catch-all route for debugging - place this LAST
